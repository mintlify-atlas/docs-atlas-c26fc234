...
<Note>
  `RES_CLEAN` is returned by commands that have no user-visible output (for example, internal housekeeping). Frontends skip printing for this type.
</Note>
...
RadishDB is deliberately single-threaded. One goroutine — or rather, one OS thread — handles everything: the event loop, command execution, expiry sweeping, and I/O.
